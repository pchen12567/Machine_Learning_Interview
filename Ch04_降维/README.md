...
引入拉格朗日乘子，并对$w$求导令其等于0，便可以推出$ \Sigma w = \lambda w $，此时：
...
LDA首先是为了分类服务的，因此只要找到一个投影方向$w$，使得投影后的样本尽可能安装原始分类分开。
...
定义类间散度矩阵为：
$$ S_B = w^T (\mu_1 - \mu_2)(\mu_1 - \mu_2)^T w $$
定义类内散度矩阵为：
$$ S_w = \sum_{x \in C_i} w^T (x - \mu_i)(x - \mu_i)^T w $$
则$J(w)$可以写成：
...
对于二分类这一问题，由于$S_B = (\mu_1 - \mu_2)(\mu_1 - \mu_2)^T$，因此$S_B w$的方向始终与$(\mu_1 - \mu_2)$一致，
如果只考虑$w$的方向，不考虑其长度，可以得到$w = S_w^{-1}(\mu_1 - \mu_2)$。换句话说，只需要求样本的均值和类内方差，
就可以马上得出最佳的投影方向。这便是Fisher在1936年提出的线性判别分析。
